...
resource "aws_instance" "myEc2Public" {
  ...
  user_data              = <<-EOF
              ...
              unzip awscliv2.zip
              sudo ./aws/install
              aws ecr-public get-login-password --region us-east-1 | docker login --username AWS --password-stdin public.ecr.aws/y4e4l6z4
              sudo docker push public.ecr.aws/y4e4l6z4/mkreddy:my
              sudo docker run -d -p 8080:80 public.ecr.aws/y4e4l6z4/mkreddy:my
              EOF
